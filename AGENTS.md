# DailyDose Development Guidelines

Auto-generated from all feature plans. Last updated: 2026-04-22

## Active Technologies
- Kotlin on JVM 17, Android app with minSdk 24 / targetSdk 36 + Jetpack Compose Material3, Paging 3, Hilt, Firebase Realtime Database, Firebase Storage, Coil, AndroidX Room (003-snapshot-reactions-replies)
- Firebase Realtime Database for snapshots/reactions/replies and summary fields; Firebase Storage for post images; Room plus app-private files for retained feed and offline action state (003-snapshot-reactions-replies)
- Firebase Realtime Database for snapshots, daily prompt assignment, and user posting metadata; Firebase Storage for post images; Room plus app-private files for retained feed and offline home state (004-daily-prompt-posting)
- Firebase Realtime Database for snapshots, daily prompt assignment, user posting metadata, reactions/replies, and per-user reveal records; Firebase Storage for post images; Room plus app-private files for retained feed, offline action state, and cached reveal visibility (005-blur-to-reveal)

- Kotlin on JVM 17, Android app with minSdk 24 / targetSdk 36 + Jetpack Compose Material3, Paging 3, Hilt, Firebase Realtime Database, Firebase Storage, Coil, planned AndroidX Room for local persistence (003-offline-feed-support)

## Project Structure

```text
src/
tests/
```

## Commands

# Add commands for Kotlin on JVM 17, Android app with minSdk 24 / targetSdk 36

## Code Style

Kotlin on JVM 17, Android app with minSdk 24 / targetSdk 36: Follow standard conventions

## Recent Changes
- 005-blur-to-reveal: Added Kotlin on JVM 17, Android app with minSdk 24 / targetSdk 36 + Jetpack Compose Material3, Paging 3, Hilt, Firebase Realtime Database, Firebase Storage, Coil, AndroidX Room
- 004-daily-prompt-posting: Added Kotlin on JVM 17, Android app with minSdk 24 / targetSdk 36 + Jetpack Compose Material3, Paging 3, Hilt, Firebase Realtime Database, Firebase Storage, Coil, AndroidX Room
- 003-snapshot-reactions-replies: Added Kotlin on JVM 17, Android app with minSdk 24 / targetSdk 36 + Jetpack Compose Material3, Paging 3, Hilt, Firebase Realtime Database, Firebase Storage, Coil, AndroidX Room


<!-- MANUAL ADDITIONS START -->
<!-- MANUAL ADDITIONS END -->
